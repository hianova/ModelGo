...
impl Vec101Engine {
    pub fn new(model_path: &str, config: crate::config::EngineConfig) -> std::io::Result<Self> {
        ...
        Ok(Self { loader, safetensors_loader: None, tokenizer, config })
    }
    ...
    pub fn generate_shared_parallel(&mut self, shared_kv: &crate::tiered_kv::TieredKVCache, prompts: &[String; 3]) -> crate::decision_engine::DecisionResult {
        let batch_size = 3;
    ...
    pub fn classify_logits(&mut self, prompt: &str, candidate_tokens: &[u32]) -> u32 {
        ...
        // Post-forward: Only check the logits of candidate tokens
        let mut best_token = candidate_tokens[0];
        let mut best_logit = f32::NEG_INFINITY;
        
        for &token in candidate_tokens {
            if (token as usize) < 4096 {
                let logit = out_buffer[token as usize];
                if logit > best_logit {
                    best_logit = logit;
                    best_token = token;
                }
        ...
        println!("[Logit Classification] Classified '{}' without generation. Picked Token {}", prompt, best_token);
        best_token
    }
    ...
    pub fn query_with_page_fault(&mut self, query: &str) -> Result<String, String> {
        ...
        if let (Some(path), Some(mut meta)) = (target_file, target_metadata) {
            if meta.status == "unprocessed" {
                ...
                let kv_cache = crate::tiered_kv::TieredKVCache::new(target_file_id, 2048, 16, &self.config);
