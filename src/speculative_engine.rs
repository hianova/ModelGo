use crate::loader::{ArchivedSerializedLayerData, ZeroCopyModelLoader};
use std::sync::Arc;
use vec101::{types::QuantType, vec101_compute, vec101_context};
...
impl SpeculativeEngine {
    pub fn new(
        loader: Arc<ZeroCopyModelLoader>,
        max_batch_size: usize,
        hidden_dim: usize,
        session_id: u32,
    ) -> Self {
        Self {
            kv_cache: TieredKVCache::new(
                session_id,
                hidden_dim,
                64,
            ...
            s_stream: vec![1.0; 256000],
            out_buffer: vec![0.0; max_batch_size * 256000],
            loader,
    ...
    pub unsafe fn run_draft_mode(
        ...
        for _depth in 0..target_depth {
            ...
            for (idx, layer) in layers.iter().enumerate() {
                ...
                let (quant_type, w_stream, blocks_per_row, num_rows) = match &layer.data {
                    ArchivedSerializedLayerData::Bit1_58(blocks) => {
                        let bpr = self.kv_cache.hidden_dim / 2048;
                        (
                            QuantType::Bit1_58,
                    ...
                    ArchivedSerializedLayerData::Q4_0(blocks) => {
                        let bpr = self.kv_cache.hidden_dim / 32;
                        (
                            QuantType::Q4_0,
                            blocks.as_ptr() as *const u8,
                            bpr,
                ...
                let ctx = vec101_context { hardware_handle: std::ptr::null_mut(),
                    ...
                    num_rows,
                    blocks_per_row,
                    num_threads: 0, // FIX: Disable dynamic thread spawning to prevent OS thrashing and deadlocks
    ...
    pub unsafe fn run_verify_mode(
        ...
        for (idx, layer) in layers.iter().enumerate() {
            ...
            let ctx = vec101_context { hardware_handle: std::ptr::null_mut(),
                ...
                num_rows,
                blocks_per_row,
                num_threads: 0, // FIX: Disable dynamic thread spawning
    ...
    fn test_speculative_engine() {
        std::thread::Builder::new()
            ...
            .spawn(|| {
                ...
                if let Ok(loader) = ZeroCopyModelLoader::new(temp_file.path()) {
                    let arc_loader = Arc::new(loader);
                    let mut engine = SpeculativeEngine::new(arc_loader, 1, 16, 42);

                    unsafe {
                        let tree = engine.run_draft_mode(1, 1, 1);
                        assert!(!tree.tokens.is_empty());
                        let _verified = engine.run_verify_mode(&tree, 1);
                    }
                }
            })
            .unwrap()
