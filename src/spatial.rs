...
impl SpatialIntentEngine {
    ...
    pub fn trigger(bbox: BoundingBox, annotation: &str) -> Result<()> {
        println!(
            "\n[Spatial Intent] Triggered bounding box capture at (X:{}, Y:{}, W:{}, H:{})",
            bbox.x, bbox.y, bbox.width, bbox.height
        );
        println!(
            "[Spatial Intent] Micro-Annotation received: \"{}\"",
            annotation
        );
        ...
        let prompt = format!(
            "Given the visual annotation '{}' inside bounding box (X:{}, Y:{}, W:{}, H:{}), output the data extraction script or action.",
            annotation, bbox.x, bbox.y, bbox.width, bbox.height
        );
        ...
        if annotation.to_lowercase().contains("csv") {
            ...
            println!(
                "[Action Injection] Result physically saved to {}",
                target_path.display()
            );
        }
    ...
    fn test_spatial_intent_engine() {
        std::thread::Builder::new()
            ...
            .spawn(|| {
                ...
                let result = SpatialIntentEngine::trigger(bbox, "extract to csv");
                assert!(result.is_ok());
                ...
                let result2 = SpatialIntentEngine::trigger(bbox2, "normal annotation");
