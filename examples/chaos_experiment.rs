...
fn main() {
    ...
    for _ in 0..num_simulations {
        ...
        if chaos_attempts < min_chaos {
            min_chaos = chaos_attempts;
        }
        if chaos_attempts > max_chaos {
            max_chaos = chaos_attempts;
        }
    ...
    println!(
        "Average Attempts to Learn: {:.2}",
        (linear_attempts_total as f64) / (num_simulations as f64)
    );
    ...
    println!(
        "Average Attempts to Learn: {:.2}",
        (chaos_attempts_total as f64) / (num_simulations as f64)
    );
    ...
    for (i, &count) in chaos_distribution.iter().enumerate().take(20).skip(1) {
        if count > 0 {
            let percentage = (count as f64 / num_simulations as f64) * 100.0;
    ...
    println!(
        "when extreme Zipf outliers occur, bridging deterministic code with human-like 'Aha!' moments."
    );
}
